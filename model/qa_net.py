...
# File Name: RNet
# Edit Author: lnest
# The content of this file is mostly copied from https://github.com/andy840314/QANet-pytorch-
# ------------------------------------
...
class InitializedConv1d(nn.Module):
    ...
    def forward(self, x):
        return F.relu(self.out(x)) if self.relu is True else self.out(x)
...
class Highway(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            ...
            tran = self.linear[i](x)
            tran = F.dropout(tran, p=DROPOUT, training=self.training)
            x = gate * tran + (1 - gate) * x
        return x
...
class SelfAttention(nn.Module):
    def __init__(self):
        super().__init__()
        self.mem_conv = InitializedConv1d(D, D*2, kernel_size=1, relu=False, bias=False)
        self.query_conv = InitializedConv1d(D, D, kernel_size=1, relu=False, bias=False)
...
class Embedding(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1d = InitializedConv1d(Dword, D, bias=False)
        self.high = Highway(2)
...
class Pointer(nn.Module):
    def __init__(self):
        super().__init__()
        self.w1 = InitializedConv1d(D*3, 1)
        self.trans = nn.Linear(LC, 3)
...
class QANet(nn.Module):
    def __init__(self, word_mat):
        ...
        self.cq_resizer = InitializedConv1d(D * 4, D)
    ...
    def forward(self, Cwid, Qwid):
        ...
        for i, blk in enumerate(self.model_enc_blks):
             M0 = blk(M0, maskC, i*(2+2)+1, 7)
        M3 = M0
        p1 = self.out(M1, M2, M3, maskC)
        return F.softmax(p1, dim=1)

    def predict(self, Cwid, Qwid):
        p = self.forward(Cwid, Qwid)
